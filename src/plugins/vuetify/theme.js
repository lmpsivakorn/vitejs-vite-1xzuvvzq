...
const vuetifyDefaultColors = Object.keys(colors)
  .flatMap(color => Object.keys(colors[color])
    .map(opt => ({
      [`${color.replace(/([A-Z])/, v => '-' + v.toLowerCase())}${opt === 'base' ? '' : '-' + opt.replace(/(\d)/, v => '-' + v)}`]: colors[color][opt]
    }))
  ).reduce((all, cur) => ({...all, ...cur}), {})
vuetifyDefaultColors['white'] = '#FFFFFF'
vuetifyDefaultColors['black'] = '#000000'
